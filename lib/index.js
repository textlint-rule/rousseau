...
function rousseau(text, options, callback) {
    ...
    checks = _.chain([options.language, _.head(options.language.split("_"))])
        .uniq()
        .reduce(function (_checks, lng) {
            return _checks || languages[lng];
        }, null)
        .omitBy(function (check, name) {
            // Check if only this one
            if (options.only.length > 0 && !_.includes(options.only, name)) return true;
