...
describe("Sentences Tokeniser", function () {
    ...
    it("should handle abbreviation (1)", function () {
        var sentences = english.sentences()(
    ...
    it("should extract correctly sentences from HTML (simple)", function () {
        var html = tokenizeHtml(
            '<p>On <b>Jan. 20</b>, former <a href="#">Sen. Barack Obama</a> became the 44th President of the U.S. Millions attended the Inauguration.</p>'
        );
    ...
    it("should correctly merge tokens and split into sentences", function () {
        ...
        sentences[0].value.should.equal(
            'When an x86-based computer is turned on, it begins a complex path to get to the stage where control is transferred to our kernel\'s "main" routine ().'
        );

        sentences[1].index.should.equal(220);
        sentences[1].offset.should.equal(99);
        sentences[1].value.should.equal(
